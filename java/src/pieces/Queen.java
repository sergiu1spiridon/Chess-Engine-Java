...
import common.Pair;
import interfaces.QueenInterface;
import xboard.Moves;
...
public class Queen implements QueenInterface {
	...
	public Queen(Pair position) {
		this.position = position;
	}

	/**
	 * Makes a new string from the chessboard and new potential position
	 * @param newX
	 * @param newY
	 * @return
	 */
...
//		charArray[position.y * 9 + position.x + 1] = '*';
		...
		if (ok == 1) {
			if (XBoard.getInstance().isPlayingWhite()) {
				charArray[position.y * 9 + position.x + 1] = 'q';
			} else {
				charArray[position.y * 9 + position.x + 1] = 'Q';
			}
		}
